...
open class ElasticModalViewController: UIViewController, ElasticMenuTransitionDelegate {
  ...
  open var dragDownTransformType:ElasticTransitionBackgroundTransform = .subtle
  open var dragRightTransformType:ElasticTransitionBackgroundTransform = .translatePull
 ...
 open lazy var modalTransition:ElasticTransition = {
  ...
  open var dismissByForegroundDrag:Bool {
  ...
  open func dismissFromTop(_ sender:UIView?){
    modalTransition.edge = .bottom
    modalTransition.transformType = dragDownTransformType
  ...
  open func dismissFromLeft(_ sender:UIView?){
  ...
  open func handleLeftPan(_ pan:UIPanGestureRecognizer){
    if pan.state == .began{
      modalTransition.transformType = dragRightTransformType
      modalTransition.edge = .right
